...
var autoprefixer, browserify, buffer, coffee, concat, errorify, gulp, gutil, merge, minifycss, npmpackage, rename, replace, source, sourcemaps, stylus, svgmin, svgstore, uglify, watchify;
...
gulp.task('svg', function() {
  ...
  return merge(flat, color).pipe(svgstore()).pipe(rename(npmpackage.name + "-" + npmpackage.version + ".min.svg")).pipe(gulp.dest('dist'));
});
...
coffee = function(options) {
  ...
  compressor = function() {
    ...
    return comp.pipe(sourcemaps.write('./')).pipe(gulp.dest('dist'));
  };
  if (shouldwatch) {
